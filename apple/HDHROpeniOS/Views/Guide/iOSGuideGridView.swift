...
///
/// The ruler does not scroll in lockstep with the program track, and there is no auto-scroll-to-"now"
/// on initial load: both were prototyped using `ScrollViewReader` (for scroll-to-anchor) and a
/// `GeometryReader`/`PreferenceKey` pair (for offset sync), but wrapping the horizontally-scrolling
/// track in a `ScrollViewReader` alone -- independent of the offset-sync mechanism, which was ruled
/// out separately -- reproduced the same continuous-CPU-pegging bug seen elsewhere in this view.
/// Dropped rather than worked around; the ruler still shows the correct initial window, and users can
/// scroll horizontally to "now" manually.
